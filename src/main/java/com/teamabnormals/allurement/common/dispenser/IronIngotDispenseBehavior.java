...
public class IronIngotDispenseBehavior extends OptionalDispenseItemBehavior {
	...
	public static void spawnParticles(ServerLevel level, SimpleParticleType particleType, RandomSource random, BlockPos pos) {
		for (int i = 0; i < 12; ++i) {
			...
			NetworkUtil.spawnParticle(level, particleType, Collections.singletonList(new ParticleInstance(d6, d7, d8, d2, d3, d4)));
		}
	}
	...
	public static void repairAnvil(Block result, Level level, BlockPos facingPos) {
		...
		if (level instanceof ServerLevel serverLevel) {
			if (random.nextInt(AllurementConfig.COMMON.ingotRepairChance.get()) == 0) {
				...
				spawnParticles(serverLevel, ParticleTypes.FIREWORK, random, facingPos);
			} else {
				spawnParticles(serverLevel, ParticleTypes.SMOKE, random, facingPos);
			}
		}
